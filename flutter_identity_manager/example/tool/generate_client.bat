...
pause
openssl rsa -RSAPublicKey_in -in input.pem -pubout -out pkcs8.pem
openssl x509 -req -in fake.csr -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -sha512 -days 365 -force_pubkey pkcs8.pem -out client-cert.pem
del fake.csr
del pkcs8.pem
